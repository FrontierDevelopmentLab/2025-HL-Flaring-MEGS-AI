...
def train():
   ...
   # print(f"Training Seq2Seq regression: input_length={input_seq_length}, output_length={output_seq_length}, stride={stride}")
    ...
    data_loader.setup()
    ## Oversampling
    ## Check if oversampling is enabled in config_updated.yaml
    use_oversampling = config_data["training"].get("use_oversampling", False)
    if use_oversampling:
        print("Oversampling enabled: applying WeightRandomSampler on training data...")
        ## Extract final SXR value from training sequences
        last_sxr_values = [float(sxr_seq[-1]) for _, sxr_seq in data_loader.train_ds]
        log_sxr = np.log10(np.array(last_sxr_values)+1e-8)

        ## Bin the log10(SXR) values
        num_bins = 10
        bins = np.linspace(log_sxr.min(), log_sxr.max(), num_bins + 1)
        bin_indices = np.digitize(log_sxr, bins[:-1], right=True)
        class_counts = np.bincount(bin_indices, minlength=num_bins+2)
        ...
        ## Replace train_loader with oversampled version
        data_loader.train_dataloader = lambda: torch.utils.data.DataLoader(
            data_loader.train_ds,
            batch_size=config_data["training"]["batch_size"],
            sampler = sampler,
            num_workers = os.cpu_count()
        )
        print("WeightedRandomSampler applied. Sampled class imbalance corrected!")
    else:
        print("Oversampled Disabled: using default shuffled sampling")

    ## Visualize oversampling distribution (W&B)
    if wandb.run is not None:
        sampled_log_sxr = log_sxr[list(sampler)]
        plt.figure(figsize=(12, 10))
        plt.hist(log_sxr, bins =20, color="blue", alpha = 0.6, label= "original")
        plt.hist(sampled_log_sxr, bins=20, color="orange", alpha=0.6, label="oversampled")
        ## add title
        plt.title("Original verus Oversampled log10(SXR) Distribution")
        plt.xlabel("log10(SXR)")
        plt.ylabel("Count")
        plt.legend()
        wandb.log({"Oversampling/Distribution": wandb.Image(plt)})
        plt.close()
